...
export default function App() {
    ...
    useEffect(() => {
        fetch("http://127.0.0.1:9001/host-info")
            ...
            .then(systemInfo => {
                console.log("Host info:", systemInfo);  // 👈 DEBUG

                setData(prev => ({
                    ...prev,
                    host_system: systemInfo
                }));
            })
            .catch(err => {
                    ...
                    <div className="header">
                                ...
                                <Upload
                                    onResult={(result) =>
                                        setData(prev => ({
                                            ...prev,
                                            ...result,
                                            host_system: prev.host_system  // preserve host info
                                        }))
                                    }
                                />
                            </div>
                        ...
                        <div className="host-panel">
                            ...
                            <div className="host-stats">
                                <div className="stat">
                                    <span>OS</span>
                                    <strong>{data.host_system?.os}</strong>
                                </div>

                                <div className="stat">
                                    <span>Architecture</span>
                                    <strong>{data.host_system?.architecture}</strong>
                                </div>

                                <div className="stat">
                                    <span>CPU Cores</span>
                                    <strong>{data.host_system?.cpu_cores}</strong>
                                </div>

                                <div className="stat">
                                    <span>Total RAM</span>
                                    <strong>{data.host_system?.total_ram_gb} GB</strong>
                                </div>

                                <div className="stat">
                                    <span>Available RAM</span>
                                    <strong>{data.host_system?.available_ram_gb} GB</strong>
                                </div>

                                <div className="stat">
                                    <span>Disk</span>
                                    <strong>{data.host_system?.total_disk_gb} GB</strong>
                                </div>
                            </div>
                        </div>
                    </div>
